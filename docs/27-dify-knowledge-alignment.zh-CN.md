# 27 Dify 知识库处理与检索兼容性基线

> 当前规范：本文记录 OpenKB 与 Dify External Knowledge 配合时的知识库处理、分块、检索、QA、摘要和 segment 管理兼容性基线。已验证的本地 Dify 1.14.1 升级记录见 `docs/28-dify-1.14.1-knowledge-gap-audit.zh-CN.md`；分块与检索兼容性测试基线见 `docs/30-dify-parity-v2-analysis.zh-CN.md`；Phase 23-25 收口记录见 `docs/31-dify-parity-next-phases.zh-CN.md`。若本文中状态与 docs/30 冲突，以 docs/30 的工程基线为准。

本文件记录 OpenKB 对照 Dify `1.14.1` 源码后的知识库处理、分块、检索和 segment 管理映射。Dify 源码只作为公开实现参照，临时放在仓库外目录，不进入 OpenKB git。

> 状态收口：Phase 22.2-22.6 的基础能力、22.8 的 Dify 兼容 splitter、22.9 的主要兼容性测试收敛、Phase 23 的配置一致性、Phase 24 的 QA 兼容语义和 Phase 25 的图片/附件检索底座已经进入当前主线。后续验证入口统一写入 `docs/31-dify-parity-next-phases.zh-CN.md`。
...
| `process_rule.mode=automatic` | 自动分段 | `process_rule_mode` + Dify 1.14.1 recursive splitter 默认值 | Phase 22.8 已作为新 reprocess 默认；Phase 22.9 已补兼容性 fixture 输出 |
| `process_rule.mode=custom` | 自定义分段 | fixed separator + recursive fallback | Phase 22.8 已兼容 splitter 行为；Phase 22.9 已补 raw/Milkdown/indexed 对照 |
| `process_rule.mode=hierarchical` | 父子分段 | fixed parent/subchunk separator + recursive fallback | Phase 22.8 已兼容 splitter 行为；Phase 22.9 已补 parent/full-doc focused fixtures |
...
| `weights` | hybrid 权重 | Dify 风格 keyword/vector 权重映射到 Milvus WeightedRanker | Phase 22.3 已实现 |
...
- `knowledge_base_chunk_settings` 增加 Dify 风格默认规则：`doc_form`、`indexing_technique`、`process_rule_mode`、`process_rule`、`retrieval_model`、`summary_index_setting`。
...
- KB Dashboard 顶层保留 Overview / Segments / Retrieval Lab / Settings；Settings 内按 Dify 风格信息层级拆为处理模式、分块规则、检索策略、metadata、摘要、重处理。
...
- Phase 22.8 已将新建或显式 reprocess 的 chunking 默认切到 Dify 兼容 splitter。
...
### Phase 22.8 / 22.9 兼容性测试基线

- `docs/30` 是当前 splitter/retrieval/metadata/QA/segment 兼容性测试工程基线；Phase 22.9 已把 QA 对外内容、tags 文档 metadata post-filter 和 focused fixture 证据纳入验收表。
