...
> 本文是工程路线和验证入口，不是产品宣传口径。README 和当前用户入口统一使用“Dify 配合 / 兼容 / 接入”；本文中的 `parity` 仅指本地兼容性测试脚本和历史运行目录。

本文原本是 Phase 22.9 之后的后续路线入口；现在作为 Phase 23-25 的收口记录和后续验证入口。Phase 22.2-22.6 已完成 Dify 风格处理规则、显式 reprocess、retrieval model、segment 管理、QA/summary 和 Web 信息层级；Phase 22.8-22.9 已完成 Dify 兼容 splitter 与兼容性测试主要收敛。

Phase 23-25 已按原顺序推进：先补低风险兼容与配置一致性，再补 QA 兼容语义，最后做图片与附件检索。Phase 25 稳定收口时已完成同模型 live retrieval 兼容性测试复跑和 image-capable smoke；后续重点转为发布后观察、排序差异归因和旧派生数据的手动重建 runbook。

当前收口修复也纳入 Phase 25：只读用户可以进入文档 `分段` 页查看 segments、QA、summary 和处理快照；Dify Admin 选择 KB 时会刷新建议的 External Knowledge ID 并重置 allowed KB scope；创建知识库的 `doc_form` 非法值会返回 `INVALID_INPUT`。

## 当前剩余项

- **同模型 live retrieval 兼容性测试**：已在 Phase 25 本机环境复跑完成。100 篇 corpus、240 条查询、qwen3-vl embedding/rerank、hybrid + rerank 同开关；证据路径见 `docs/30-dify-parity-v2-analysis.zh-CN.md`。
...
目标：按 Dify 1.14.1 的 multimodal/attachment 逻辑完善图片和附件检索兼容性。

- 状态：已完成底座闭环。
- Markdown `asset://` 图片/附件会建立 `document_asset_bindings`；外部 http(s) 图片只做文本 metadata 索引，不抓取远程图片，避免 SSRF。
- Derived asset chunk 通过 `source_chunk_id` 回到源 segment，metadata 使用 Dify 风格字段：`doc_type`、`segment_attachment_id`、`attachment_info`、`asset_preview_url/source_url`、`asset_match_text`、`image_vector_enabled/fallback_reason`。
...
- OpenKB 与 Dify 配合时保留 Markdown-first：正文仍是 `document_versions.markdown`，图片、QA、summary、segment 都是检索派生层。
...
- Phase 23：chunk 参数 round-trip、snapshot 过期标记、三种 doc_form reprocess、Dify splitter 兼容性测试。
...
- Live retrieval 兼容性测试：Dify/OpenKB 同 corpus、同 embedding/rerank、同 top_k/threshold/rerank/hybrid 开关后复跑 top-k overlap、MRR/nDCG、score 差异和归因；Phase 25 证据见 `.codex-runtime/parity-runs/20260517T135537Z/retrieval/`。
