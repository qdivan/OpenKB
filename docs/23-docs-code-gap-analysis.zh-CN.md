...
- 文档中规划的后续能力尚未进入 v0.3.x，例如生产 SMTP、完整 MCP OAuth、复杂 OCR/MinerU、dense/hybrid/rerank、实时协同、Admin UI。
...
| `01-product-vision.zh-CN.md` | 项目定位、Markdown-first、MCP、Dify、私有化部署方向已对齐。 | 愿景中的 PDF/DOCX/PPTX/XLSX/图片导入、dense/sparse/hybrid/rerank、完整分享/审批体验仍是后续增强；当前导入只启用 Markdown/Text/HTML/CSV，检索默认 BM25/text-only。 |
...
| `09-search-rag-milvus-native.zh-CN.md` | Milvus schema builder、`id` primary key + `chunk_id` regular field、BM25 Function、TEXTEMBEDDING/RERANK Function 占位、alias rebuild/switch、PostgreSQL final check 已实现。 | 当前检索执行路径是 BM25 sparse search；dense/hybrid/rerank 生产链路未接入。手动新建文档默认 `draft`，而 Milvus/search filter 只返回 `doc_status = "published"`，目前缺显式发布 API。 |
...
| `11-dify-adapter.zh-CN.md` | `/retrieval`、Bearer scoped API key、knowledge mapping、metadata_condition post-filter、Dify 错误格式、审计、top_k 限制已实现。 | Dify key 管理只有 CLI/helper，没有 Web 管理页；检索仍依赖当前 Milvus BM25 index，dense/hybrid/rerank 未接入。 |
...
| `14-ui-routes.zh-CN.md` | `/login`、`/register`、`/verify-email`、`/app`、workspace/KB/doc/search 路由已实现。 | `/share/:token` 和 `/admin/*` 页面未实现；Share/Collaborator panel 未实现；知识库设置页未实现；Admin Milvus UI 未实现。 |
...
5. 后续如果要进入产品增强阶段，应优先补分享/协作者 UI、Admin UI、复杂导入 adapter、MCP OAuth 和 Key 管理 UI。
