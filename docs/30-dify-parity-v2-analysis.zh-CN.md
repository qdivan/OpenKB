...
- `standard_custom_space` 的长证书/base64 类无空格文本也已对齐字符级 fallback/overlap。
...
| Dify-compatible splitter | Phase 22.8 起，automatic/custom/hierarchical 默认使用 Dify 1.14.1-compatible splitter；长度使用 Unicode code point 语义，减少 CJK/emoji 与 JS UTF-16 `.length` 偏差 |
| `chunkMarkdownParentChild()` | paragraph/full-doc 都可表达，parent/subchunk 均走 Dify-compatible splitter |
...
OpenKB 的优势仍是权限与派生命中解释更稳：summary、QA、source chunk 都会回到 PostgreSQL 终检。Dify parity 修复不能把 Milvus 或 Dify metadata 提升为权限真相。
...
5. **同模型检索环境仍需单独复跑**
   报告指出 Dify 环境没有完整启用 Milvus hybrid。必须在 Dify/OpenKB 使用同一 embedding/rerank/hybrid 开关后，才讨论 top-k、score、rerank 是否真正对齐。
...
| P1 | Dify-compatible splitter 默认行为 | 已验证 | 100 篇公开 Markdown、9 种模式复跑均为 100/100 文档完全一致；证据目录 `.codex-runtime/parity-runs/20260515T125253Z/` |
...
| P1 | QA 语义对齐 | 已实现 | Dify Adapter QA 命中返回 `question:... answer:...` 内容，并带 `hit_type=qa`、`qa_question`、`qa_answer`、`qa_pair_id`、`source_chunk_id` |
